...
// Заправка (шаг 3): кончик уходит ЗА столб (TUCK3_BACK — слой back), выныривает слева,
// заходит ПОД DA (TUCK3_UNDER — рисуется до DA, контур DA его прерывает), затем
// ВЫНЫРИВАЕТ за DA и торчит наружу (TUCK3_OVER — рисуется после DA, поверх, с маркером).
const TUCK3_BACK = 'M245,440 L165,420';
const TUCK3_UNDER = 'M165,420 L196,402';
const TUCK3_OVER = 'M229,382 L256,366';

// Финал (шаг 4): ЗАТЯНУТО, но НА ТОМ ЖЕ МЕСТЕ (центр ≈200,375). Тот же крест, диагонали
// короче на ~12% к центру; камера не двигается. WTOP4 выходит культями за края столба.
const DB4 = 'M156,441 L244,309'; // виток 1, чуть короче, центр тот же
const DA4 = 'M156,309 L240,432'; // виток 2, чуть короче, перекрест по центру
const WTOP4 = 'M248,309 L152,309'; // верхний обход за столбом, культи за края опоры
const KOR4 = 'M156,441 C148,530 150,595 150,648'; // коренной — снизу-СЛЕВА
const TUCK4_BACK = 'M240,432 L165,415'; // кончик уходит ЗА столб (слой back)
const TUCK4_FRONT = 'M165,415 L246,373'; // выныривает, ПОД DA, торчит справа — отдельно от коренного
...
const step3 = frame(
  ...
  halo(213, 392, 45) +
    rope(KOR, KORENNOY_MUTED) +
    rope(TUCK3_UNDER, HODOVOY) +
    rope(DB, HODOVOY_MUTED) +
    rope(DA, HODOVOY_MUTED) +
    rope(TUCK3_OVER, HODOVOY) +
    marker(256, 366),
);
